...
- **Restore planning is now available as a read-only command**:
  `duplicacy-backup restore plan --storage <storage> <label>` resolves the
  selected label and storage, shows storage/config/secrets/state context, and
...
- **S3-compatible Duplicacy schemes now load storage credentials**:
  `s3c://`, `minio://`, and `minios://` schemes require the same
  `s3_id` and `s3_secret` keys as `s3://`, so local object-storage entries
...
- **Redundant `storage_type` output has been removed**: runtime JSON summaries,
  health reports, and notification payloads now report storage identity through
  `storage`, `location`, and configured storage value rather than repeating the
  retired type model.
...
- **Duplicacy backend configuration is now provider-neutral**: backup storage entries
  use a direct `storage` value, and storage secrets now use generic
  `[storage.<name>.keys]` entries that are passed through to Duplicacy
  preferences.
- **Config parsing and operator guidance now use current schema terms**:
  single-storage fallback detection is named explicitly, storage health override
  resolution is simpler, and missing required value errors point to
  `common.*` / `storage.<name>.*` keys.

### Removed
- **The Storj/S3-specific object storage schema has been removed**:
  `type = "object"` plus `storj_s3_id` / `storj_s3_secret` is replaced by the
  standardised `storage` plus generic storage keys model.
- **The storage `type` key has been retired**: every storage entry delegates storage
  handling to Duplicacy, so operator config now uses only `location` and
  `storage` for storage identity.

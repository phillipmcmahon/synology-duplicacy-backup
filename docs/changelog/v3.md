...
- **Operator output now starts with explicit identity everywhere**: runtime,
  ...
  `label + storage` at the start of operator-facing output, making local and
...
- **Runtime wording is more precise**: `fix-perms` now labels the resolved
  storage path as `Destination`, avoiding the old collision where `Storage`
  could mean either the named storage entry or the filesystem path.
...
- **Pre-run failures now carry useful run scope**: runtime failures that occur
  ...
  `Operation`, `Label`, and `Storage` before the error and final result block.
...
- **Stronger config validation preflight**: `config validate` now actively
  ...
  writability, remote destination resolution, storage settings, health
...
- **Examples now reinforce the explicit `label + storage` model**: release
  docs and cheat-sheet comments consistently name both the label and storage
  being acted on, without implying any default or hierarchy between storage.
...
### Added
- **Named storage operating model**: One backup label can now define multiple
  named storage entries inside a single `<label>-backup.toml` file, with one matching
  `<label>-secrets.toml` file for any storage credentials. This makes
  configurations such as `homes/onsite-usb` and `homes/offsite-storj`
  first-class without forcing separate label files.
- **Storage-specific runtime identity**: Health and run state now live under
  `/var/lib/duplicacy-backup/<label>.<storage>.json`, and machine-readable JSON
  now carries explicit `label` and `storage` identity for both run and health
  summaries.
- **Short operator cheat sheet for the new model**: The desk cheat sheet and
  install examples now document the explicit named-storage workflow end to end,
  including config, secrets, health, and scheduler usage.

### Changed
- **CLI model is now fully explicit**: Every runtime, `config`, and `health`
  command now requires `--storage <name>`, and runtime commands must also pass
  ...
  storage guessing has been removed.
- **Configuration is now per label rather than per label-storage file**: The
  preferred operational layout is now one config file per label with shared
  `[common]` and `[health]` settings plus one or more `[storage.<name>]`
  tables. Secrets follow the same label-level structure with storage-scoped
  entries under `[storage.<name>]`.
- **Operator language polished around the new storage model**: Human-facing
  output now consistently uses natural storage-oriented wording such as
...
- **Machine JSON now preserves exact user storage names**: `mode` and `storage`
  now use the exact lower-case storage name supplied in config, rather than
...
- **Release-facing docs aligned to the new architecture**: README, CLI,
  ...
  internal walkthrough now describe the same explicit named-storage operating
  model.
- **Coverage raised for release readiness**: The storage-first refactor now has
...
### Fixed
- **Independent storage histories now stay independent**: Local and offsite
  storage entries under the same label no longer share runtime state, health recency,
  or revision expectations, so natural drift between destinations is handled
  correctly.
- **Snapshot naming collisions removed for same-label storage entries**: Snapshot names
  now include the storage name plus a unique suffix, avoiding same-second collisions
  when different storage entries for the same label run close together.
- **Locking now matches real operational boundaries**: The source snapshot
  ...
  storage-scoped, allowing safer same-label multi-storage behaviour.
- **Interactive and JSON output now tell the same story**: Final wording,
  duration rendering, and storage labelling are aligned across terminal output,
...
- **Release recommendation**: This is a substantial operator-facing
  ...
  storage model, with release notes calling out the explicit `--storage`

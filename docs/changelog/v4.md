...
### Added
- **Local object-storage entries are now supported**: storage entries can use
...
### Changed
- **Storage documentation now separates backend mechanics from operational
...
- **Global self-update notifications**: `update` can now send failure
  ...
  label/storage backup settings and without reading storage secrets.
...
- **Update notification config stays global by design**: update alerts use
  ...
  storage entry, or object-storage secrets unless a future authenticated notification
...
- **Optional notification-token parsing now has less duplication and better
  ...
  exercises meaningful edge cases around missing storage entries, missing tokens,
...
- **Simulated notification sends are now a first-class operator workflow**:
  `duplicacy-backup notify test --storage <name> <label>` now sends a clearly
  marked synthetic notification through the configured providers for the
  selected storage, with support for `--provider`, `--severity`, `--summary`,
...
- **Notification test guidance now explains what the command proves**: the
  ...
  simulated sends validate provider delivery and auth for the selected storage,
...
- **Config-command parse failures no longer try to open `/var/log` first**:
  request and config-command parse errors such as missing `--storage` now report
...
- **Non-root `config validate` is now more truthful on protected systems**:
  ...
  object-storage secrets validation, and repository probes now report
...
### Changed
- **Storage entries now use an explicit storage-kind and deployment-location model**:
  storage config now uses `type = "filesystem" | "object"` plus
...
- **Mounted remote filesystems are now first-class**: destinations such as
  SMB-over-VPN paths can now be modelled as `filesystem/remote` storage entries
  without loading object-storage secrets or pretending to be local storage.
- **Operator output now surfaces the new storage model consistently**: runtime,
  ...
  and reports the storage-model check under `Storage Settings`.

### Fixed
- **Pre-run failure context is now fully aligned with the new storage model**:
  when config can be resolved before a run aborts, pre-run failures now show
  `Type` and `Location` alongside `Operation`, `Label`, and `Storage`.

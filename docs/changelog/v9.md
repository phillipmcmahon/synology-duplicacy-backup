...
- Removed the retired `allow_local_accounts`, `local_owner`, and `local_group`
  storage config keys from the active config model, docs, examples, and tests.
...
- **NAS UI surface smoke**: `109` captures, `0` unexpected outcomes, and
  dual-storage restore smoke with `onsite-garage` plus root-protected
...
- Restore drill workspace names can now be configured with
  ...
  `{storage}`, `{snapshot_timestamp}`, `{revision}`, and `{run_timestamp}` so
  operators can recover the same revision from different storage entries without
  clobbering or obscuring prior drills.
- UI surface smoke restore automation now creates a smoke-owned restore root,
  runs a workspace-template dry-run matrix, supports multiple restore storage
  entries with per-storage sudo policy, and verifies restored data exists after the real
  optional restore.
...
- **Linux Go 1.26**:
  `go run honnef.co/go/tools/cmd/staticcheck ./...`
- **Linux Go 1.26**: `go test -cover ./...`
- **NAS UI surface smoke**: dual-storage restore smoke with
...
- Remote mounted filesystem repositories no longer inherit the sudo-required
  policy for root-protected local filesystem repositories; storage entries with

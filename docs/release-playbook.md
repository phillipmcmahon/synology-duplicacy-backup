...
- significant package coverage from the generated release notes, especially
  architecture packages such as `internal/workflowcore`, `internal/restore`,
  `internal/health`, and `internal/operator`

If these numbers changed materially, explain the movement in the release notes.
For example, call out a coverage decrease when new registry, package-boundary,
or orchestration code paths were added before all follow-up tests landed, and
link the story that tracks the backfill. If the movement changes the documented
test baseline, update `TESTING.md`.
...
## Operator impact
- State whether CLI, config-file, scheduling, restore, update, or privilege
  behaviour changed. If the release is internal-only, say so explicitly.
...
## Further reading
- Link the changelog entry or docs that explain the release rationale.
```
...
- Include the important shipped story from any failed or superseded release
  attempts.
- Frame material coverage movement honestly. Do not hide a drop; explain why
  it happened and whether follow-up test work is tracked.
- Include the significant package coverage lines generated by release prep
  unless there is a clear reason to omit a package from the public notes.
- Link relevant docs when a release highlights architecture, release-process,
  privilege-model, or operator-workflow changes.

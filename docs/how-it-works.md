...
`restore files` are read-only, `restore select` is an interactive directory
and file guide unless `--execute` is explicitly confirmed, `restore prepare`
only creates the separate drill workspace and writes Duplicacy preferences
there, and `restore run` executes Duplicacy only inside that prepared
workspace.

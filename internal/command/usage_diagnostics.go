...
Examples:
    ...
    {{script}} diagnostics --storage offsite-storj --json-summary homes
...
BEHAVIOUR:
    diagnostics:
      - resolves one label and storage without running backup, prune, restore, or storage maintenance
...
EXAMPLES:
    {{script}} diagnostics --storage onsite-usb homes
    {{script}} diagnostics --storage offsite-storj homes
    {{script}} diagnostics --storage offsite-storj --json-summary homes
    {{script}} diagnostics --storage onsite-usb --config-dir /opt/etc --secrets-dir /opt/secrets homes
`,
		"{{default_secrets_dir}}", defaultSecretsDirDisplay(rt),
		"{{config_dir}}", cfgDir,

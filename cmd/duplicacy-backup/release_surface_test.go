...
func TestReleaseDocs_StayAlignedWithCurrentSurface(t *testing.T) {
	...
	expectations := map[string][]string{
		...
		filepath.Join(root, "docs", "operations.md"): {
			...
			"written and",
			"tested for Synology DSM only",
			"must already be initialized with the",
			"Duplicacy CLI",
		},
		filepath.Join(root, "docs", "quickstart.md"): {
			"backup on Synology DSM",
			"does not initialize new Duplicacy repositories",
